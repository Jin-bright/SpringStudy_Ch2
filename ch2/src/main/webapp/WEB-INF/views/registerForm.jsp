...
<body>
   <form action="<c:url value="/register/save" />" method="post" onsubmit="return formCheck(this)" name="frm">
   ...
   <script>
       function formCheck(frm) {
            ...
            if(frm.id.value.length<3) { //아이디 체크 
                setMessage('id의 길이는 3이상이어야 합니다.', frm.id);
                return false;
            }
            
            if(frm.pwd.value.length<4) { //pw 체크
       ...
       function setMessage(msg, element){
            ...
            if(element) {
                element.select(); // 값을 잘못입력했을 때 그 잘못 입력한 값이 선택되게 하는것 
            }
       }
   </script>
</body>
